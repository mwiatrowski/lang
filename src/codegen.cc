...
std::string writeTemporaryAssignment(std::ostream &output,
                                     const AstNodeExpr &expr);

std::string generateFuncCallStr(std::ostream &output,
                                const AstNodeFuncCall &funcCall) {
...
std::string writeTemporaryAssignment(std::ostream &output,
  ...
  if (const auto literal = to<AstNodeStringLiteral>(expr)) {
    auto escapedStr = "R\"IMPL_STR_LITERAL(" +
                      std::string{literal->value.value} + ")IMPL_STR_LITERAL\"";
...
void writeAssignment(std::ostream &output,
