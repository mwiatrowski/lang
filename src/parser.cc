...
std::pair<std::optional<AstNodeExpr>, TokensSpan> consumeExpression(TokensSpan tokens);

std::pair<std::optional<AstNodeStmt>, TokensSpan> consumeStatement(TokensSpan tokens);

using TypedArgList = decltype(AstNodeFuncDef::arguments);

std::pair<std::optional<TypedArgList>, TokensSpan> consumeTypedArgList(TokensSpan tokens) {
...
std::pair<std::optional<AstNodeFuncDef>, TokensSpan> consumeFunctionDefinition(TokensSpan tokens) {
...
std::pair<std::optional<AstNodeFuncCall>, TokensSpan> consumeFunctionCall(TokensSpan tokens) {
    ...
    while (!tokens.empty()) {
        ...
        if (peek<TokenRBrace>(tokens)) {
            std::tie(std::ignore, tokens) = consume<TokenRBrace>(tokens);
            auto funcCall = AstNodeFuncCall{.functionName = funcName, .arguments = std::move(args)};
            return {funcCall, tokens};
        }
...
std::pair<std::optional<AstNodeExpr>, TokensSpan> consumeBasicExpression(TokensSpan tokens) {
    if (peek<TokenIdentifier, TokenLBrace>(tokens)) {
        auto [funcCall, tokensTail] = consumeFunctionCall(tokens);
        auto expr = funcCall.has_value() ? std::optional<AstNodeExpr>{*funcCall} : std::optional<AstNodeExpr>{};
    ...
    if (peek<TokenKwFn>(tokens)) {
        ...
        auto expr = funcDef.has_value() ? std::optional<AstNodeExpr>{*funcDef} : std::optional<AstNodeExpr>{};
        return {expr, tokensTail};
    }
...
std::pair<std::optional<AstNodeStmt>, TokensSpan> consumeAssignment(TokensSpan tokens) {
    ...
    auto assignment = AstNodeAssignment{.variable = std::move(*variable), .value = std::move(*expr)};
